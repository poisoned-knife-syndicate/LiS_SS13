...
/datum/game_mode/proc/process_autoantag()
	message_admins("[ruppertext(name)]: Attempting autospawn.")

	var/list/usable_templates = list()
	for(var/datum/antagonist/A in antag_templates)
		if(A.can_late_spawn())
			message_admins("[ruppertext(name)]: [A.id] selected for spawn attempt.")
			usable_templates |= A

	if(!usable_templates.len)
		message_admins("[ruppertext(name)]: Failed to find configured mode spawn templates, please re-enable auto-antagonists after one is added.")
		round_autoantag = 0
		return
	...
	while(usable_templates.len)
		...
		if(spawn_antag.attempt_auto_spawn())
			message_admins("[ruppertext(name)]: Auto-added a new [spawn_antag.role_text].")
	...
	message_admins("[ruppertext(name)]: Failed to proc a viable spawn template.")
