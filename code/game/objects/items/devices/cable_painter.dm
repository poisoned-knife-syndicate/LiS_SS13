...
obj/item/device/cable_painter/examine(var/user)
	. = ..(user, 1)
	if(.)
		to_chat(user, "The color is currently set to [rlowertext(color_selection)].")
...
obj/item/device/cable_painter/attack_self(mob/user)
	...
	if(new_color_selection && !user.incapacitated() && (src in user))
		...
		to_chat(user, "<span class='notice'>You change the paint mode to [rlowertext(color_selection)].</span>")
...
/obj/item/device/cable_painter/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(istype(A, /obj/structure/cable))
		...
		to_chat(user, "<span class='notice'>You set \the [A]'s color to [rlowertext(color_selection)].</span>")
	else if(isCoil(A))
